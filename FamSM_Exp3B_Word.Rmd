...
Forty eight `pseudowords` were repeated 30 times on Day 1. Only `pseudowords` were used in the current experiment. Each of 13 participants completed a total of 1,440 trials, divided into 10 blocks of 144 trials each. Each item was repeated three times within a block. In each trial, participants indicated whether it was the first, second, or third time the given item was presented within the block. Two participants (5th & 6th) did not complete the session due to a technical issue. These two participants were exposed to each item 24 to 27 times.
...
Two participants (6th & 7th) did not make responses in all or most of trials in Block 1. In subsequent blocks, participants made faster correct responses while maintaining a similar level of accuracy. Overall accuracy was `r round(mean(cD1s$M), digits=2)`% (SD = `r round(sd(cD1s$M), digits=2)`). 
...
The three-phase procedure as in Experiments 1 and 2 was conducted on Day 2. In the item repetition phase, half of the pseudowords learned on Day 1 were repeated eight time, producing 192 trials. In each trial, participants made a preference judgment for an item.
...
The mean of individuals' preference scores was `r round(mean(P1slong$Preference)*100, digits=2)`%. In the following plot, red point and error bar represent the means and 95% bootstrapped CIs.
...
Participants learned 48 pairs of a pseudoword and a quadrant on the screen. They were instructed to pay attention to the location of each face while making a preference judgment. All pseudowords shown on Day 1 were presented once in one of four quadrants. The within-participant **item repetition** was the single factor. Twenty four pseudowords had been repeated in the first phase (`repeated`) while the other 24 faces had not (`unrepeated`).
...
We calculated the mean and *s.d.* of individual participants' accuracy rates for each condition. As in Experiment 2, preference for `pseudowords` increased after `repetition`. In the following plot, red points and error bars represent the means and 95% CIs.
...
Mean preference scores were submitted to a one-way repeated measures ANOVA. Participants preferred the pseudowords of the `repetition` condition significantly to those of the `no repetition` condition (*familiarity benefit*). 
...
We calculated the mean and *s.d.* of individual participants' mean percentage accuracy. Source memory accuracy was greater in the `unrepeated` condition than the `repeated` condition (*novelty benefit*). In the following plot, red points and error bars represent the means and 95% within-participants CIs. 
...
The *p*-values from all models agreed strongly providing a high degree of confidence in the results. Item repetition impaired source memory of `pseudowords` that were familiarized on the previous day. 
...
The following table presents the mean and *s.d.* of individual participants' confidence ratings in each condition. The difference between conditions appears negligible. 
...
To determine the significance of the fixed effect, the LRT compared the full model (`cm.full`) with its nested model (`cm.red1`) without the effect of interest. The next table showed that The effect of **item repetition** was not significant.
...
Since the overall accuracy of source memory was not high, only small numbers of correct trials were available after trimming. The following table summarizes the numbers of RTs submitted to subsequent analyses. None of participants had more than 20 trials per condition. 
...
We calculated the mean and s.d. of individual participants’ mean RTs. The numerical pattern of RTs was consistent with the accuracy and confidence data. Participants made slightly faster responses in the `repeated` condition than `unrepeated` condition.
