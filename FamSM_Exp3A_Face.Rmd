...
Forty eight faces were repeated 30 times on Day 1. Only `non-famous` faces were used in the current experiment. Each of 13 participants completed a total of 1,440 trials, divided into 10 blocks of 144 trials each. Each face was repeated three times within a block. In each trial, participants indicated whether it was the first, second, or third time the given face was presented within the block. 
...
Three participants (1st, 7th, & 9th) did not make responses in all or most of trials in Block 1. The performance improved over time; accuracy increased and RT decreased in subsequent blocks. Overall accuracy was `r round(mean(cD1s$M), digits=2)`% (SD = `r round(sd(cD1s$M), digits=2)`).
...
The three-phase procedure as in Experiments 1 and 2 was conducted on Day 2. In the first phase, 24 faces out of the 48 faces learned on Day 1 were repeated eight time, producing 192 trials. In each trial, participants made a male/female judgment for a face. 
...
Participants performed the task very wel. The mean accuracy was `r round(mean(P1slong$Accuracy), digits=2)`%. In the following plot, red point and error bar represent the means and 95% bootstrapped CIs.
...
Participants learned 48 pairs of a face and a quadrant on the screen. They were instructed to pay attention to the location of each face while making a male/female judgment. All faces shown on Day 1 were presented once in one of four quadrants. The within-participant **item repetition** was the single factor. Twenty four faces had been repeated in the first phase (`repeated`) while the other 24 faces had not (`unrepeated`).
...
We calculated the mean and *s.d.* of individual participants' accuracy rates for each condition. There was only negligible difference between `repeated` vs. `unrepeated` conditions. In the following plot, red points and error bars represent the means and 95% CIs.
...
Mean percentage accuracy was submitted to a one-way repeated measures ANOVA. The difference between conditions was not statistically significant.
...
Source memory accuracy was greater in the `unrepeated` condition than the `repeated` condition (*novelty benefit*). 
...
The *p*-values from both models agreed strongly providing a high degree of confidence in the results. Item repetition impaired source memory of `non-famous` faces once they had become familiar on the previous day. 
...
The following table presents the mean and *s.d.* of individual participants' confidence ratings in each condition. The pattern of confidence ratings was qualitatively identical to those from accuracy analysis; we observed the *novelty benefit* for `non-famous` faces. 
...
To determine the significance of the fixed effect, the LRT compared the full model (`cm.full`) with its nested model (`cm.red1`) without the effect of interest. The next table showed that **item repetition** significantly reduced confidence on source memory. 
...
We calculated the mean and *s.d.* of individual participants’ mean RTs. The numerical pattern of RTs was consistent with the accuracy and confidence data. Participants made slightly faster responses in the `unrepeated` condition than `repeated` condition.
