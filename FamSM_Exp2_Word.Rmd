...
Thirty participants were recruited. Half of participants watched 24 `words` while the other half watched 24 `pseudowords` (between-participants factor, **pre-experimental item familiarity**). The faces were repeated eight times, producing 192 trials. In each trial, participants made a preference judgment for an item. 
...
We calculated the mean and *s.d.* of individual participants' preference scores in each condition. Participants showed greater preference for `words` than `pseudowords`. In the following plot, red points and error bars represent the means and 95% CIs.
...
Mean preference scores were submitted to a one-way ANOVA. The difference between conditions was statistically significant. The preference score was higher in the `words` than `pseudowords` conditions.
...
Participants learned 48 pairs of an item (word or pseudoword) and a quadrant on the screen. They were instructed to pay attention to the location of each item while making a preference judgment. A between-participants **pre-experimental item familiarity** factor determined whether a participant watched `words` or `pseudowords`. A within-participant **item repetition** factor determined whether a trial presented a `repeated` item, which had appeared in the first phase or an `unrepeated` item, which appeared for the first time in the experiment.
...
We calculated the mean and *s.d.* of individual participants' preference scores in each condition. After repetition, preference decreased for `words` but increased for `pseudowords`.
...
We performed a 2x2 mixed design ANOVA with **pre-experimental item familiarity** as a between-participants factor and **item repetition** as a within-participant factor. The main effect of **pre-experimental item familiarity** and the two-way interaction were significant. 
...
As post-hoc analyses, we performed two one-way repeated measures ANOVAs to contrast `repeated` vs. `unrepeated` items within each **pre-experimental item familiarity** condition. The first table shows the results in the `words` condition and the second table in the `pseudowords` condition.
...
Item repetition did not affect preference significantly for `pseudowords`.
...
The effect of **item repetition** depended on the **pre-experimental familiarity** of the items. For `words`, the source of unrepeated items was better remembered (*novelty effect*). For `pseudowords`, the source of repeated items was better remembered (*familiarity effect*). These crossed effects were on top of the general familiarity effect, in which source memory was more accurate for `words` than for `pseudowords`. 

### ANOVA

Individuals' mean percentage accuracy was submitted to a 2x2 mixed design ANOVA with **pre-experimental item familiarity** as a between-participants factor (`words` vs. `pseudowords`) and **item repetition** as a within-participant factor (`repeated` vs. `unrepeated`).
...
In the **pseudoword** group, source memory was marginally more accurate for **repeated** than **unrepeated** items. The 95% CI of difference between the means was [`r round(ci95[1], digits=2)`, `r round(ci95[2], digits=2)`].
...
The *p*-values from both models agreed strongly providing a high degree of confidence in the results. Now we proceeded to the post-hoc analyses. As in the following table, the results from the pairwise comparisons were consistent with those based on the ANOVA. Item repetition significantly impaired source memory of `words` whereas it slightly improved source memory of `pseudowords`. 
...
The next table presents the mean and *s.d.* of individual participants' confidence ratings in each condition. The pattern of confidence ratings was qualitatively identical to those from accuracy analysis; we observed the **novelty benefit** with `words` and the **familiarity benefit** with `pseudowords`.
...
Individuals’ confidence ratings were submitted to a 2x2 mixed design ANOVA with **pre-experimental item familiarity** as a between-participants factor and **item repetition** as a within-participant factor.
...
The main effect of **pre-experimental item familiarity** and the two-way interaction were significant. The main effect of **item repetition** were marginally significant. Next we performed two additional one-way repeated measures ANOVAs as post-hoc analyses. The first table below presents the effect of item repetition for `words`, and the second presents the same effect for `pseudowords`. The results evidenced that the repetition of items induced significant confidence changes in an opposite direction for `words` vs. `pseudowords` (see below).
...
In the **word** group, the participants were more confident on their source memory for **unrepeated** than **repeated** items. The 95% CI of difference between the means was [`r round(ci95[1], digits=2)`, `r round(ci95[2], digits=2)`].
...
In the **pseudoword** group, the participants' confidence ratings were higher for **repeated** than **unrepeated** items. The 95% CI of difference between the means was [`r round(ci95[1], digits=2)`, `r round(ci95[2], digits=2)`].
...
Since the overall accuracy of source memory was not high, only small numbers of correct trials were available after trimming. The following table summarizes the numbers of RTs submitted to subsequent analyses. None of participants had more than 25 trials per condition. 
...
The effect of **pre-experimental item familiarity** was significant. Participants made faster responses to `words` than `pseudowords`. The two-way interaction was also significant. Next, we proceeded to post-hoc analyses with two one-way repeated measures ANOVAs. The first table below presents the effect of item repetition for `words`, and the second table presents the same effect for `pseudowords`. 
...
For `words`, the pattern of earlier responses for unrepeated words was significant.
...
For `pseudowords`, the pattern of earlier responses for repeated pseudowords was marginally significant. 
...
The preference scores in Phase 2 resembled the source memory accuracy and confidence ratings in Phase 3. Does preference determine source memory? Here, we reanalyzed the data from Phase 3 by considering participants' preference in Phase 2 as an additional factor. 
...
Individuals’ mean percentage accuracy was submitted to a 2x2x2 mixed design ANOVA with **pre-experimental item familiarity** (`words` vs. `pseudowords`) as a between-participants factor and **item repetition** (`repeated` vs. `unrepeated`) and **preference** (`like` vs. `dislike`)as two within-participant factors. The main effect of **preference** and its interactions with the other factors were not significant.
...
Individuals’ onfidence ratings were submitted to a 2x2x2 mixed design ANOVA with **pre-experimental item familiarity** (`words` vs. `pseudowords`) as a between-participants factor and **item repetition** (`repeated` vs. `unrepeated`) and `preference` (`like` vs. `dislike`)as two within-participant factors. The main effect of **preference** was significant. More importantly, however, **preference** did not significantly interact with the other factors. 
...
For both accuracy and confidence of source memory, no interactions with **preference** reached statistical significance. In contrast, the interaction between **pre-experimental item familiarity** and **item repetition** was always significant. These results evidenced that preference alone cannot account for the main findings on source memory.
